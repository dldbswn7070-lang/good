# 🧑‍💻 Coding Study Log (꾸준한 기록)

매일 성장하는 데이터 분석가가 되기 위해 **SQL 쿼리**와 **파이썬 프로그래머스**와 **백준** 문제를 풀고 기록하는 저장소입니다. 단순히 정답을 맞히는 것을 넘어, 효율적인 코드와 논리적인 사고를 기르는 것을 목표로 합니다.
...
```text
📦 Coding-Study
 ├── 📂 Python            #(수동) 강의 학습 및 문법 기초 정리
 │    ├── 📂 Basic        # 문법 연습 문제 풀이 
 │    └── 📂 Programmers  # 초기에 수동으로 푼 문제들
 ├── 📂 SQL             # (수동) SQL 문제 풀이
 │    ├── 📂 Level1     # SELECT, WHERE, ORDER BY 기본
 │    ├── 📂 Level2     # GROUP BY, 집계함수, 간단한 JOIN
 │    ├── 📂 Level3     # 서브쿼리, 복잡한 JOIN, CASE WHEN
 │    ├── 📂 Level4     # 윈도우 함수, CTE, 복잡한 서브쿼리
 │    └── 📂 Level5     # 고급 분석, 재귀 쿼리, 복잡한 로직
 │ 
 ├── 📂 programmers     # (자동) 파이 코딩테스트 연습 (Auto-upload)
 │    ├──📂 L0_기초입문
 │    │   ├── 입문         # 입문 / 기초 트레이닝
 │    │   └── 기초트레이닝
 │    ├──📂 L1_입문       # Level 1 문제
 ...
 │    ├──📂 L7_챌린지     # Level 7 문제
 │    └──📂 other              # 미등록 문제
 ├── 📂baekjoon          # (자동) 백준 알고리즘 (Auto-upload)
 │     ├──📂 L8_백준      # Level 8 문제
 │     └──📂 other             # 미등록 문제    
 └── 📄 README.md
```
